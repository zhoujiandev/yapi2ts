...
import { YapiProject, YapiCategory, YapiInterface, YapiInterfaceDetail, YapiResponse } from './types';
...
export class YapiService {
   ...
   * 获取分类下的接口列表
   */
  async getInterfaceList(catid: number, page: number = 1, limit: number = 20): Promise<{
    total: number;
    count: number;
    list: YapiInterface[];
  }> {
