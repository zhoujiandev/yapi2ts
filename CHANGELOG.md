...
- **代码折叠功能** - 在代码预览弹框中新增了像编辑器一样的代码折叠功能
    - 引入了高效的括号 `{}` 与块注释 `/* ... */` 范围扫描引擎，自动识别可折叠区块。
    - 支持高仿真缩进引导线（Indent Guidelines），竖线精准跟随代码缩进层级移动并在闭合大括号处收尾，且去除了最左侧冗余竖线，使代码层级结构一目了然。
    - 折叠块支持收起/展开，收起时显示可点击展开的 `...` 占位标识。

### Improved

- **代码预览弹框 UI/UX 深度升级** - 重构顶部与底部视觉设计，大幅提升整体精致感与操作手感
    - **顶部头部与标签页优化**：扩大内边距并引入 VS Code 原生 Codicon SVG 关闭按钮；重构“预览/编辑”切换栏为现代化胶囊式分段控件（Segmented Control）。
    - **底部操作与按钮反馈**：将按钮圆角升级为符合现代美学的 `6px`，并引入微动效（悬停时轻微上浮及阴影变深、点击时物理下压按键回弹），使操作充满高级感。
    - **无界化边栏设计**：去除了行号区域右侧贯穿首尾的灰色实线边框，使行号列与代码区无缝衔接，最大化贴近真实编辑器质感。
    - **小箭头触控热区优化**：折叠小箭头 SVG 尺寸调大至 `18px`，占位槽调整至 `28px`，操作体验更加轻松自如。
    - **折叠按钮全局联动显现**：鼠标悬停在侧边栏行号/折叠区域时，可一键半透明显现所有折叠小箭头，便于快速定位与操作。
